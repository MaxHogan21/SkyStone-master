...
//@Autonomous(name = "Blue_Turn_5")
public class Blue_Turn_5 extends LinearOpMode {
        ...
        */
        do {
            ...
            if (vuMark == RelicRecoveryVuMark.LEFT) {
                ...
                rightStrafe(1.2,1,1500);

                driveStop(500);
                //Turns so Robot is now facing the CryptoBox
                rightTurn(1,750);
                driveStop(100);
                if(getRelativeHeading()>-90){
    ...
    public void turnTimelessR(double value) {
        NATARI.leftf.setPower(-value);
        NATARI.rightf.setPower(value);
        NATARI.leftb.setPower(-value);
        NATARI.rightb.setPower(value);
     ...
     */
    public int gyroTurnR(double valueTarget, double valueError, double valueActual, double minSpeed, double maxSpeed) {
        ...
        if (Math.abs(changeValue) > valueError) { //makes delta positive and makes sure it is large than the error
            ...
            turnTimelessR(minSpeed * Math.signum(scaleValue) + maxSpeed * scaleValue);
        }
        else {
            NATARI.varCount++;
    ...
    public int gyroTurnL(double valueTarget, double valueError, double valueActual, double minSpeed, double maxSpeed) {
        double changeValue = (valueTarget - valueActual + 360.0) % 360.0; //This finds the difference between the two values then the remainder
        if (changeValue > 180.0) changeValue -= 360.0; //changes the delta between -180 and 180 degrees
        if (Math.abs(changeValue) > valueError) { //makes delta positive and makes sure it is large than the error
            NATARI.varCount = 0;
